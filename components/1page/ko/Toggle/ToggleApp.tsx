...
const ToggleApp = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        for (let i = 0; i < response.data.data.length; i++) { //경기, 강원 ... //name,provinceLink,cityList[]
          districtIndex.current = i;
          if (i === 0) {
            for (var j = 0; j < response.data.data[i].cityList.length; j++) {
              console.log(response.data.data[i].cityList);
  ...
  if (districtIndex.current === 0) {
    settingCity(gyeongi, gyeongiindex, stationList);
      ...
      settingCity(chungnam, chungnamindex, stationList);
  } else if (districtIndex.current === 3) {
      settingCity(chungbuk, chungbukindex, stationList);
  } else if (districtIndex.current === 4) {
      settingCity(jeonnam, jeonnamindex, stationList);
  } else if (districtIndex.current === 5) {
      settingCity(jeonbuk, jeonbukindex, stationList);
  } else if (districtIndex.current === 6) {
      settingCity(gyeongnam, gyeongnamindex, stationList);
  } else if (districtIndex.current === 7) {
      settingCity(gyeongbuk, gyeongbukindex, stationList);
  } else if (districtIndex.current === 8) {
      settingCity(incheon, incheonindex, stationList);
  } else if (districtIndex.current === 9) {
      settingCity(daejeon, daejeonindex, stationList);
  } else if (districtIndex.current === 10) {
      settingCity(daegu, daeguindex, stationList);
  } else if (districtIndex.current === 11) {
      settingCity(busan, busanindex, stationList);
  } else {
      settingCity(gwangju, gwangjuindex, stationList);
  }
    ...
    const gangwon_list = gangwon.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === gangwon.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
    ...
    const chungnam_list = chungnam.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === chungnam.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
    ...
    const chungbuk_list = chungbuk.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === chungbuk.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
    ...
    const jeonnam_list = jeonnam.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === jeonnam.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
    ...
    const jeonbuk_list = jeonbuk.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === jeonbuk.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
    ...
    const incheon_list = incheon.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === incheon.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
    ...
    const daejeon_list = daejeon.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === daejeon.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
    ...
    const gwangju_list = gwangju.city.map((v: string, index: number) => (
      ...
      >
        <li className="city_li"
            style={map === gwangju.city[index].city_link ? { fontSize: '1.2em' } : { fontSize: '' }}>
          ...
          <div className="nextbutton">
            {selectedcity !== '없음' ? (
              <button className="gotosecondbtn" id="citycatedone">
                <Link to={`/${city_engtokor[selectedcity]}`}
