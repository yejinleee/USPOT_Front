...
const Thumbnail : FC<Props> = ({children, selectedcity,selectedcategory,btn_pic }) => {
  ...
  var dic : { [key :string]:number} = {
    ...
    '강릉시':15,'춘천시':16,'정선군':17,'삼척시':18,'태백시':19,'동해시':20,'원주시':21,'횡성군':22,'철원군':23,'평창군':24,'영월군':25,
    '논산시':26,'계룡시':27,'공주시':28,'홍성군':29,'보령시':30,'아산시':31,'서천군':32,'천안시':33,'예산군':34,
    '영동군':35,'단양군':36,'청주시':37,'충주시':38,'제천시':39,'증평군':40,'옥천군':41,'음성군':42,
    '곡성군':43,'광양시':44,'순천시':45,'나주시':46,'화순군':47,'보성군':48,'목포시':49,'무안군':50,'장성군':51,'함평군':52,'여수시':53,
    '군산시':54,'김제시':55,'익산시':56,'전주시':57,'정읍시':58,'완주군':59,'임실군':60,'남원시':61,
    '창원시':62,'밀양시':63,'진주시':64,'하동군':65,'김해시':66,'사천시':67,'양산시':68,'함안군':69,
    '영덕군':70,'구미시':71,'예천군':72,'경주시':73,'경산시':74,'청도군':75,'봉화군':76,'영천시':77,'상주시':78,'군위군':79,'영주시':80,'포항시':81,'의성군':82,'문경시':83,'칠곡군':84,'안동시':85,'김천시':86,
    '인천':87,'대전':88,'대구':89,'부산':90,'울산':91,'광주':92,'세종':93,
  };
  ...
  return (
    <>
      <div className="youtube_all">
        여기에 동영상이 나와여
         {vloglist !==[] && vloglist.map((datas: any, i: any) => (
           <div className="youtube_each">
             <iframe width="336" height="188" src={`https://www.youtube.com/embed/${datas}?controls=0`}
                     title="YouTube video player" frameBorder="0"
                     allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                     allowFullScreen></iframe>
